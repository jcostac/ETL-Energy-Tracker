...
def test_esios_full_etl(test_date):
    ...
    transform_result = transformer.transform_data_for_all_markets(test_date, test_date, mode='single')
    assert transform_result['success'], "ESIOS transformation failed"
...
def test_i90_full_etl(test_date):
    ...
    transform_result = transformer.transform_data_for_all_markets(
        test_date, test_date, dataset_type='precios_i90', transform_type='single'
    )
    assert transform_result['status']['success'], "I90 transformation failed"

    loader = LocalDataLakeLoader()
    load_result = loader.load_transformed_data_precios_i90(transform_result)
