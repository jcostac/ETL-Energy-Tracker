...
class IncrementalChangeMonitor:
    ...
    async def _trigger_linking_for_specific_ups(self, ups_to_link: List[str], 
            ...
            try:
                # Perform linking process for the specified UPs
                results = await self.linking_algorithm.link_uofs_to_ups(target_date, ups_to_link=ups_to_link)

                if results['success'] and not results['links_df'].empty:
                    new_links = results['links_df']
                    # Format for database
                    ...
                    if not results['success']:
                        print(f"⚠️  Linking process failed: {results['message']}")
                    else:
    ...
    def _save_new_links(self, engine, new_links_df: pd.DataFrame):
        """
        Saves the newly created links to the database.
        
        Args:
            engine: Database engine
            new_links_df: DataFrame with new links to save
        """
        if new_links_df.empty:
            print("ℹ️  No new links to save.")
            return

        try:
            table_name = self.config.UP_UOF_VINCULACION_TABLE
            print(f"\n💾 Saving {len(new_links_df)} new links to `{table_name}`...")
            
            self.db_utils.write_table(engine, new_links_df, table_name, if_exists='append')
            
            print(f"✅ Successfully saved new links.")
        except Exception as e:
            print(f"❌ Error saving new links to database: {e}")

        #main method to run the incremental check       
    async def run_incremental_check(self, engine, check_date: str) -> Dict:
        """
        ...
        try:
            ...
            if enabled_ups:
                ...
                if unlinked_ups:
                    ...
                    new_links = await self._trigger_linking_for_specific_ups(unlinked_ups, target_date, engine)
                    
                    # Step 4: Save new links to the database
                    if not new_links.empty:
                        self._save_new_links(engine, new_links)
